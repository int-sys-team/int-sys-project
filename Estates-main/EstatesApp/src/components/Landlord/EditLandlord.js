...
export default function EditStudentProfileDialog({
    ...
    async function handleSubmit() {
        ...
        const response = await fetch(
            "http://localhost:5100/api/Landlord/EditLandlord/" +
            id,
            {
                method: "PUT",
                credentials: "include",
                ...
                body: JSON.stringify({ id: id, name: name, contact: contact, propertyIds: Properties, reviews: Reviews })
            }
        )
        ...
        if(!response.ok) {
            alert("[Error occured] Could not update landlord")
            return
        }
    };
